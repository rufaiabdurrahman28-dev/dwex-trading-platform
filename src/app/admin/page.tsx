...
import { Users, TrendingUp, DollarSign, Activity, CheckCircle2, AlertCircle, Clock, Shield, Search, Eye } from 'lucide-react'
...
const overviewStats = [
  { label: 'Total Users', value: '12,847', change: '+5.2%', icon: Users, color: '#00A88A' },
  { label: 'Active Traders', value: '3,241', change: '+8.1%', icon: TrendingUp, color: '#00A88A' },
  { label: 'Total Volume', value: '$24.5M', change: '+12.3%', icon: DollarSign, color: '#E5940A' },
  { label: 'Total Deposits', value: '₦2.1B', change: '+7.8%', icon: Activity, color: '#00A88A' },
]
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="lg:col-span-2">
            <h2 className="text-lg font-bold mb-4 text-gray-900">Recent Transactions</h2>
            <Card className="bg-white border-gray-200 overflow-hidden shadow-sm">
              <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-gray-200"><th className="text-left py-3 px-4 text-xs text-gray-400 font-medium">User</th><th className="text-left py-3 px-4 text-xs text-gray-400 font-medium">Type</th><th className="text-right py-3 px-4 text-xs text-gray-400 font-medium">Amount</th><th className="text-center py-3 px-4 text-xs text-gray-400 font-medium">Status</th><th className="text-right py-3 px-4 text-xs text-gray-400 font-medium">Time</th></tr></thead>
                <tbody>{recentTransactions.map((tx) => (<tr key={tx.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4 text-center"><Badge variant="outline" className={cn('text-[10px] px-1.5 border-0', tx.status === 'completed' ? 'bg-[#00A88A]/10 text-[#00A88A]' : tx.status === 'pending' ? 'bg-[#E5940A]/10 text-[#E5940A]' : 'bg-gray-100 text-gray-500')}>{tx.status === 'completed' && <CheckCircle2 className="w-2.5 h-2.5 mr-0.5" />}{tx.status === 'pending' && <Clock className="w-2.5 h-2.5 mr-0.5" />}{tx.status}</Badge></td>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
          ...
          <Card className="bg-white border-gray-200 overflow-hidden shadow-sm">
            <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-gray-200"><th className="text-left py-3 px-4 text-xs text-gray-400 font-medium">Name</th><th className="text-left py-3 px-4 text-xs text-gray-400 font-medium">Email</th><th className="text-center py-3 px-4 text-xs text-gray-400 font-medium">KYC</th><th className="text-left py-3 px-4 text-xs text-gray-400 font-medium">Phase</th><th className="text-right py-3 px-4 text-xs text-gray-400 font-medium">Balance</th><th className="text-center py-3 px-4 text-xs text-gray-400 font-medium">Status</th><th className="text-right py-3 px-4 text-xs text-gray-400 font-medium">Actions</th></tr></thead>
              <tbody>{filteredUsers.map((user) => (<tr key={user.id} className="border-b border-gray-100 hover:bg-gray-50">
                <td className="py-3 px-4 font-medium text-xs text-gray-900">{user.name}</td>
                <td className="py-3 px-4 text-xs text-gray-500">{user.email}</td>
                <td className="py-3 px-4 text-center"><Badge variant="outline" className={cn('text-[10px] px-1.5 border-0', user.kyc === 'verified' ? 'bg-[#00A88A]/10 text-[#00A88A]' : user.kyc === 'pending' ? 'bg-[#E5940A]/10 text-[#E5940A]' : user.kyc === 'rejected' ? 'bg-[#E63950]/10 text-[#E63950]' : 'bg-gray-100 text-gray-500')}>{user.kyc === 'verified' && <CheckCircle2 className="w-2.5 h-2.5 mr-0.5" />}{user.kyc === 'pending' && <Clock className="w-2.5 h-2.5 mr-0.5" />}{user.kyc === 'rejected' && <AlertCircle className="w-2.5 h-2.5 mr-0.5" />}{user.kyc === 'not_started' ? 'Not Started' : user.kyc.charAt(0).toUpperCase() + user.kyc.slice(1)}</Badge></td>
                <td className="py-3 px-4 text-xs text-gray-500">{user.phase}</td>
                <td className="py-3 px-4 text-right font-mono text-xs font-medium text-gray-900">{user.balance}</td>
                <td className="py-3 px-4 text-center"><Badge variant="outline" className={cn('text-[10px] px-1.5 border-0', user.status === 'active' ? 'bg-[#00A88A]/10 text-[#00A88A]' : user.status === 'suspended' ? 'bg-[#E63950]/10 text-[#E63950]' : 'bg-gray-100 text-gray-500')}>{user.status}</Badge></td>
